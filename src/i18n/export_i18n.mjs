...
function convert() {
  ...
  const workbook = readFile(input, { sheets });
  const jsonData = {};

  Object.keys(workbook.Sheets).forEach((sheetName) => {
    const sheet = workbook.Sheets[sheetName];
    const rows = utils.sheet_to_json(sheet, { blankrows: true });
...
function transformErrMsg(sheet, row, index) {
  ...
  const httpCode = row[COL_HTTP_CODE] || '';
...
function createJsonFiles(output, fileName, jsonData) {
  ...
  writeFile(enFilePath, `${JSON.stringify(en, null, TAB_SIZE)}\n`, (error) => {
    if (error) {
      throw error;
    }
    /* eslint-disable no-console */
